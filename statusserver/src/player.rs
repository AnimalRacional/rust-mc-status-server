use std::{fmt, io::{Read, Write}, net::{SocketAddr, TcpStream}};
...
impl Player {
    ...
    fn handle_packet<T: Read>(&mut self, packet: &mut T, info: &ServerInfo) -> Result<(), PacketError> {    
        let packet_id = varint::decode_stream(packet)?;
        println!("Packet id {:?} by {}", packet_id, self.addr);
        match packet_id {
            0 => { packets::handle_status_login(packet, self, info)?; }
            1 => { packets::handle_ping(packet, self)?; }
            p => { eprintln!("Invalid packet {} sent by {}", p, self.addr); }
        };
        Ok(())
    }
    ...
    fn handle_legacy_ping(&mut self, info: &ServerInfo) -> Result<(), PacketError> {
        ...
        if packet_identifier != 0xfa {
            eprintln!("{}: Invalid legacy ping packet identifier {}", self.addr, packet_identifier);
        }
